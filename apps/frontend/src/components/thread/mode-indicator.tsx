...
const ModeLogo = memo(function ModeLogo({ 
  ...
  return (
    <span className="flex-shrink-0 relative" style={{ height: `${height}px`, width: 'auto' }}>
      ...
      <img
        src={lightSrc}
        alt={mode === 'advanced' ? 'bapX Advanced' : 'bapX Basic'}
...
export const ModeIndicator = memo(function ModeIndicator() {
  ...
  const basicModel = useMemo(
    () => modelOptions.find((m) => m.id === 'bapx/basic' || m.label === 'bapX Basic'),
    [modelOptions]
  );
  
  const powerModel = useMemo(
    () => modelOptions.find((m) => m.id === 'bapx/power' || m.label === 'bapX Advanced Mode'),
    [modelOptions]
  );
  ...
  const otherModels = useMemo(() => {
    return modelOptions.filter(
      (m) => m.id !== 'bapx/basic' && m.id !== 'bapx/power' && 
             m.label !== 'bapX Basic' && m.label !== 'bapX Advanced Mode'
    ...
    if (powerModel) {
      if (canAccessPower) {
        ...
        usePricingModalStore.getState().openPricingModal({
          isAlert: true,
          alertTitle: 'Upgrade to access bapX Advanced mode',
        });
      }
    }
