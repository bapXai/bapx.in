...
import { usebapXComputerStore, useSetIsSidePanelOpen } from '@/stores/bapx-computer-store';
...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId, isShared = false, isNew = false, preCreatedThreadId }: ThreadComponentProps) {
  ...
  const storeIsbapXAgentFn = useIsbapXAgentFn();
  ...
  const isbapXAgent = isShared ? false : storeIsbapXAgentFn;
  ...
  const openFileInComputer = usebapXComputerStore((state) => state.openFileInComputer);
  const openFileBrowser = usebapXComputerStore((state) => state.openFileBrowser);
  const setSandboxContext = usebapXComputerStore((state) => state.setSandboxContext);
  ...
  const handleModeStarterAction = useCallback(async (
    ...
    // Open the side panel to show bapXComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  const handleStarterPrompt = useCallback((prompt: string, placeholderInfo?: { start: number; end: number }) => {
    ...
    // Keep the side panel open to show bapXComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | bapX`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive Worker conversation powered by bapX`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | bapX`);
      }
