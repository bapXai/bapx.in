...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_bapx_team !== undefined) searchParams.set('is_bapx_team', params.is_bapx_team.toString());
...
export function usebapXTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'bapx-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view bapX templates');
      }

      const response = await fetch(`${API_URL}/templates/bapx-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
