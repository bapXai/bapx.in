...
interface AgentSelectionState {
  ...
  isbapXAgent: (agents: Agent[]) => boolean;
}
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultbapXAgent = agents.find(agent => agent.metadata?.is_bapx_default);
            selectedId = defaultbapXAgent ? defaultbapXAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      autoSelectAgent: (agents: Agent[], onAgentSelect?: (agentId: string | undefined) => void, currentSelectedAgentId?: string) => {
        ...
        const defaultbapXAgent = agents.find(agent => agent.metadata?.is_bapx_default);
        const agentToSelect = defaultbapXAgent || agents[0];
      ...
      isbapXAgent: (agents: Agent[]) => {
        const { selectedAgentId } = get();
        const currentAgent = selectedAgentId 
          ? agents.find(agent => agent.agent_id === selectedAgentId)
          : null;
        return currentAgent?.metadata?.is_bapx_default || selectedAgentId === undefined;
      },
    }),
    {
...
export const useIsbapXAgentFn = () => 
  useAgentSelectionStore((state) => state.isbapXAgent);
