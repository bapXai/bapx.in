...
export const usebapXComputerStore = create<bapXComputerState>()(
  devtools(
    (set, get) => ({
      ...
      clearFileState: () => {
        console.log('[bapXComputerStore] Clearing file state');
        set({
          ...initialFileState,
        });
      },
...
export const usebapXComputerFilesSubView = () =>
  usebapXComputerStore((state) => state.filesSubView);

export const usebapXComputerCurrentPath = () =>
  usebapXComputerStore((state) => state.currentPath);

export const usebapXComputerSelectedFilePath = () =>
  usebapXComputerStore((state) => state.selectedFilePath);

export const usebapXComputerFilePathList = () =>
  usebapXComputerStore((state) => state.filePathList);

export const usebapXComputerCurrentFileIndex = () =>
  usebapXComputerStore((state) => state.currentFileIndex);

// Legacy combined selector (for backward compatibility) - use individual selectors in components
export const usebapXComputerFilesState = () => ({
  filesSubView: usebapXComputerStore((state) => state.filesSubView),
  currentPath: usebapXComputerStore((state) => state.currentPath),
  selectedFilePath: usebapXComputerStore((state) => state.selectedFilePath),
  filePathList: usebapXComputerStore((state) => state.filePathList),
  currentFileIndex: usebapXComputerStore((state) => state.currentFileIndex),
});

// Actions are stable references (functions don't change)
export const usebapXComputerActions = () =>
  usebapXComputerStore((state) => ({
