...
/**
 * Check if a model ID corresponds to a bapX mode (Basic, Advanced, or Test)
 */
export function isbapXMode(modelId: string): boolean {
  // New bapX registry IDs
  if (modelId === 'bapx/basic' || modelId === 'bapx/power' || modelId === 'bapx/test' ||
      modelId === 'bapx-basic' || modelId === 'bapx-power' || modelId === 'bapx-test') {
    return true;
  }
  // Legacy: bapX Basic (Haiku 4.5)
  if (modelId.includes('claude-haiku-4-5') || modelId.includes('heol2zyy5v48')) {
    return true;
  }
  // Legacy: bapX Advanced Mode (Sonnet 4.5)
...
export function getModelProvider(modelId: string): ModelProvider {
  // Check for bapX modes first
  if (isbapXMode(modelId)) {
    return 'bapx';
  }
...
export function ModelProviderIcon({
  ...
  const iconMap: Record<ModelProvider, string> = {
    bapx: '/bapx-symbol.svg', // bapX modes use the bapX symbol
  ...
  // Special handling for bapX symbol - needs different invert behavior
  const isbapX = provider === 'bapx';
  ...
  return (
    ...
    >
      <Image
        ...
        className={cn(
          "object-contain",
          // bapX symbol: invert in dark mode (black symbol → white)
          // Other icons: invert in dark mode (black icons → white)
          isbapX ? "dark:invert" : "dark:brightness-0 dark:invert"
        )}
