...
import { usebapXComputerStore } from '@/stores/kortix-computer-store';
...
export function bapXComputer({
  ...
  log.log('[bapXComputer] Render - toolMessages:', toolMessages.length, 'currentIndex:', currentIndex);
  log.log('[bapXComputer] project:', project ? { id: project.id, name: project.name, hasSandbox: !!project.sandbox } : 'undefined');
  log.log('[bapXComputer] sandboxId prop:', sandboxId);
  ...
  log.log('[bapXComputer] currentPair:', currentPair ? 'has pair' : 'undefined');
  log.log('[bapXComputer] currentPair.toolMessage:', currentPair?.toolMessage?.message_id || 'null');
  log.log('[bapXComputer] currentPair.assistantMessage:', currentPair?.assistantMessage?.message_id || 'null');
  
  const { toolCall, toolResult, isSuccess, assistantTimestamp, toolTimestamp } = useMemo(() => {
    if (!currentPair?.toolMessage) {
      log.log('[bapXComputer] No toolMessage in currentPair, returning null');
      return { toolCall: null, toolResult: null, isSuccess: false, assistantTimestamp: undefined, toolTimestamp: undefined };
    }
    log.log('[bapXComputer] Calling extractToolCallAndResult');
