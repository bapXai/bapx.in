 ...
 * - bapX symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for bapX symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // bapX symbol always uses solid black bg with white icon
    if (usebapXSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: usebapXSymbol ? 0 : (showBorder ? 1.5 : 0),
