...
import bapXSymbolIcon from '@/assets/brand/kortix-symbol.svg';
...
/**
 * Check if a model ID corresponds to a bapX mode (Basic or Advanced)
 */
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    kortix: bapXSymbolIcon, // bapX modes use the bapX symbol
