...
async def main():
    ...
    # Create the MCP tools client with the URL of the MCP server that's accessible by the bapX instance
    mcp_tools = kortix.MCPTools(
        "http://localhost:4000/mcp/",  # Since we are running bapX locally, we can use the local URL
        "bapX",
        allowed_tools=["get_wind_direction"],
    )
    await mcp_tools.initialize()

    kortix_client = kortix.bapX(
