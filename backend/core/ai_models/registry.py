...
class ModelFactory:
    ...
    def create_basic_model(main_llm: str, custom_model: Optional[str] = None) -> Model:
        ...
        if main_llm == "kimi":
            return Model(
                ...
                provider=ModelProvider.OPENROUTER,
                aliases=["kortix-basic", "bapX Basic"],
    ...
    def create_power_model(main_llm: str, custom_model: Optional[str] = None) -> Model:
        ...
        if main_llm == "kimi":
            return Model(
                id="kortix/power",
                name="bapX Advanced Mode",
                litellm_model_id=custom_model or default_models["kimi"],
                provider=ModelProvider.OPENROUTER,
                aliases=["kortix-power", "bapX POWER Mode", "bapX Power", "bapX Advanced Mode"],
                context_window=262_144,
                capabilities=[
                    ModelCapability.CHAT,
                    ModelCapability.FUNCTION_CALLING,
