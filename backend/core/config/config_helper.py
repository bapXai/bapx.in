...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for bapX vs custom agents."""
    ...
    # Handle bapX agents with special logic
    if is_suna_default:
...
def _extract_suna_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for bapX agents - always use central config with user customizations.
    ...
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using bapX central config for agent {agent_id}")
