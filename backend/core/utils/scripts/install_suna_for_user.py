#!/usr/bin/env python3
"""
bapX Agent Installation Script for Individual Users

Simple script to install bapX agents for users by email address or account ID.

Usage:
    # Install bapX for a user by email
    python install_suna_for_user.py user@example.com
    
    # Install bapX for a user by account ID
...
class bapXUserInstaller:
    def __init__(self):
        self.db = DBConnection()
        self.service = bapXDefaultAgentService(self.db)
    ...
    async def install_for_email(self, email: str, replace: bool = False):
        ...
        if agent_id:
            print(f"✅ Successfully installed bapX agent!")
            print(f"   🤖 Agent ID: {agent_id}")
            print(f"   👤 User: {email}")
            print(f"   📦 Account: {account_id}")
        else:
            print(f"❌ Failed to install bapX agent for {email}")
    
    async def install_for_account_id(self, account_id: str, replace: bool = False):
        print(f"🚀 Installing bapX agent for account: {account_id}")
        ...
        try:
            ...
            if agent_id:
                ...
                print(f"❌ Failed to install bapX agent for account {account_id}")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Install bapX agent for a user by email or account ID",
