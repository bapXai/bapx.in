...
class bapXDefaultAgentService:
    """Simplified bapX agent management service."""
    
    def __init__(self, db: DBConnection = None):
        self._db = db or DBConnection()
        logger.debug("🔄 bapXDefaultAgentService initialized (simplified)")
    
    async def get_suna_default_config(self) -> Dict[str, Any]:
        """Get the current bapX configuration."""
        from core.config.suna_config import SUNA_CONFIG
        return SUNA_CONFIG.copy()
    
    async def install_for_all_users(self) -> Dict[str, Any]:
        """Install bapX agent for all users who don't have one."""
        logger.debug("🚀 Installing bapX agents for users who don't have them")
        ...
        try:
            ...
            if not missing_accounts:
                return {
                    "installed_count": 0,
                    "failed_count": 0,
                    "details": ["All users already have bapX agents"]
                }
            
            logger.debug(f"📦 Installing bapX for {len(missing_accounts)} users")
            ...
            for account_id in missing_accounts:
                try:
                    ...
                    success_count += 1
                    logger.debug(f"✅ Installed bapX for user {account_id}")
    ...
    async def install_suna_agent_for_user(self, account_id: str, replace_existing: bool = False) -> Optional[str]:
        """Install bapX agent for a specific user."""
        logger.debug(f"🔄 Installing bapX agent for user: {account_id}")
        
        try:
            client = await self._db.client
            
            # Check for existing bapX agent
            ...
            if existing_result.data:
                ...
                if replace_existing:
                    ...
                    logger.debug(f"Deleted existing bapX agent for replacement")
                else:
                    logger.debug(f"User {account_id} already has bapX agent: {existing_agent_id}")
                    return existing_agent_id

            # Create new agent
            agent_id = await self._create_suna_agent_for_user(account_id)
            logger.debug(f"Successfully installed bapX agent {agent_id} for user {account_id}")
            return agent_id
    ...
    async def get_suna_agent_stats(self) -> Dict[str, Any]:
        """Get statistics about bapX agents."""
        try:
            client = await self._db.client
            ...
            return {
                ...
                "note": "bapX agents always use current central configuration"
            }
    ...
    async def _create_suna_agent_for_user(self, account_id: str) -> str:
        """Create a bapX agent for a user."""
        from core.config.suna_config import SUNA_CONFIG
    ...
    async def _create_initial_version(self, agent_id: str, account_id: str) -> None:
        """Create initial version for bapX agent.
        
        Note: We don't save system_prompt, model, or agentpress_tools for bapX agents
        ...
        try:
            ...
            # For bapX agents, only save MCPs (user customizations)
            # System prompt, model, and tools are always loaded from SUNA_CONFIG
            await version_service.create_version(
                agent_id=agent_id,
                user_id=account_id,
                system_prompt="",  # Not saved for bapX - always from SUNA_CONFIG
                configured_mcps=SUNA_CONFIG["configured_mcps"],
                custom_mcps=SUNA_CONFIG["custom_mcps"],
                agentpress_tools={},  # Not saved for bapX - always from SUNA_CONFIG
                model=None,  # Not saved for bapX - always from SUNA_CONFIG
                version_name="v1",
                change_description="Initial bapX agent installation"
            )
            
            logger.debug(f"Created initial version for bapX agent {agent_id}")
            
        except Exception as e:
            logger.error(f"Failed to create initial version for bapX agent {agent_id}: {e}")
            raise
