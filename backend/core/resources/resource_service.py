...
class ResourceService:
    ...
    async def calculate_total_storage(self, account_id: str) -> Dict[str, int]:
        """
        Calculate total storage usage for an account in bytes.
        Includes Trailbase database size and Daytona sandbox disk usage.
        
        Returns:
            Dict with 'trailbase_bytes', 'sandbox_bytes', and 'total_bytes'
        """
        import os
        ...
        # 1. Measure Trailbase storage
        # Note: In a real multi-tenant SQLite setup, we might need more granular checks,
        # but for now we measure the base traildepot data directories.
        # If this is shared, we'd ideally have per-account quotas or DB files.
        traildepot_path = os.path.join(os.getcwd(), 'traildepot', 'data')
        if not os.path.exists(traildepot_path):
            # Try absolute or relative paths if current dir isn't root
            traildepot_path = "/root/Agent/suna/traildepot/data"
            
        if os.path.exists(traildepot_path):
            for root, dirs, files in os.walk(traildepot_path):
                for f in files:
                    trailbase_bytes += os.path.getsize(os.path.join(root, f))
        
        # 2. Measure Daytona sandbox storage
        resources = await self.get_account_resources(account_id, ResourceType.SANDBOX, ResourceStatus.ACTIVE)
        for res in resources:
            try:
                sandbox_id = res.get('external_id')
                if sandbox_id:
                    sandbox = await daytona.get(sandbox_id)
                    # Use disk usage from sandbox metadata if available
                    if hasattr(sandbox, 'disk') and sandbox.disk:
                        sandbox_bytes += int(sandbox.disk) * 1024 * 1024 * 1024 # Assuming GB to bytes
            except Exception as e:
                logger.warning(f"Failed to get storage for sandbox {res.get('id')}: {e}")
                
        return {
            'trailbase_bytes': trailbase_bytes,
            'sandbox_bytes': sandbox_bytes,
            'total_bytes': trailbase_bytes + sandbox_bytes
        }
