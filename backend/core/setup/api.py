...
async def initialize_user_account(account_id: str, email: Optional[str] = None, user_record: Optional[dict] = None) -> Dict:
    try:
        ...
        logger.info(f"[SETUP] Installing bapX agent for {account_id}")
        try:
            suna_service = bapXDefaultAgentService(db)
            agent_id = await suna_service.install_suna_agent_for_user(account_id)
            if not agent_id:
                logger.warning(f"[SETUP] Failed to install bapX agent for {account_id}")
        except Exception as e:
            logger.error(f"[SETUP] Error installing bapX agent for {account_id}: {e}")
            agent_id = None
...
async def handle_user_created_webhook(
    ...
    1. Initializes account (free tier subscription + bapX agent)
