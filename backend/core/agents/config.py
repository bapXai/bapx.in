    ...
    try:
        ...
        if agent_type == "suna":
            ...
            if static_config:
                ...
                logger.info(f"⏱️ [AGENT CONFIG FAST] bapX (cached type): {(time.time() - t) * 1000:.1f}ms")
                return agent_config
    ...
    try:
        if not agent_id:
            ...
            if default_agent_id:
                ...
                logger.warning(f"[AGENT LOAD] No default agent found for account {account_id}, searching for shared bapX")
...
async def _find_shared_suna_agent():
    ...
    if shared_agent:
        ...
        if admin_user_id and shared_agent['account_id'] == admin_user_id:
            logger.info(f"✅ Using system bapX agent from admin user: {agent_data.name} ({agent_data.agent_id})")
        else:
            logger.info(f"Using shared bapX agent: {agent_data.name} ({agent_data.agent_id})")
        return agent_data
    
    if admin_user_id:
        logger.warning(f"⚠️ SYSTEM_ADMIN_USER_ID configured but no bapX agent found for user {admin_user_id}")
    
    logger.error("❌ No bapX agent found! Set SYSTEM_ADMIN_USER_ID in .env")
    return None
