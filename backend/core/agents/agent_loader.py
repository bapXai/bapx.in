...
class AgentLoader:
    ...
    def _row_to_agent_data(self, row: Dict[str, Any]) -> AgentData:
        ...
        For bapX agents, always overrides name and description from SUNA_CONFIG
        regardless of what's stored in the database.
        """
        metadata = row.get('metadata', {}) or {}
        is_suna_default = metadata.get('is_suna_default', False)
        
        # For bapX agents, always use name from SUNA_CONFIG (never DB value)
        if is_suna_default:
    ...
    async def _load_suna_config(self, agent: AgentData, user_id: Optional[str] = None):
        """
        Load bapX config using static in-memory config + cached user MCPs.
        ...
        if agent.current_version_id and user_id:
            ...
            if cached_mcps:
                ...
                logger.debug(f"⚡ bapX config loaded in {(time.time() - t_start)*1000:.1f}ms (MCPs from cache)")
                return
            ...
            try:
                ...
                logger.debug(f"bapX config loaded in {(time.time() - t_start)*1000:.1f}ms (MCPs from DB, now cached)")
            except Exception as e:
                logger.warning(f"Failed to load MCPs for bapX agent {agent.agent_id}: {e}")
                agent.configured_mcps = []
                agent.custom_mcps = []
                agent.triggers = []
        else:
            ...
            agent.custom_mcps = []
            agent.triggers = []
            logger.debug(f"⚡ bapX config loaded in {(time.time() - t_start)*1000:.1f}ms (no MCPs)")
    ...
    async def _batch_load_configs(self, agents: list[AgentData]):
        ...
        try:
            ...
            # Fallback: load bapX configs only
            for agent in agents:
                if agent.is_suna_default:
                    await self._load_suna_config(agent, agent.account_id)
                    agent.config_loaded = True
