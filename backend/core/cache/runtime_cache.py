...
def load_static_suna_config() -> Dict[str, Any]:
    ...
    logger.info(f"✅ Loaded static bapX config into memory (prompt: {len(_SUNA_STATIC_CONFIG['system_prompt'])} chars)")
    return _SUNA_STATIC_CONFIG
...
async def warm_up_suna_config_cache() -> None:
    ...
    logger.info(f"✅ bapX static config loaded in {elapsed:.1f}ms (zero DB calls)")
    ...
    try:
        ...
        if not agent_ids:
            try:
                ...
                agent_ids = await agents_repo.get_user_agent_ids(user_id)
                if agent_ids:
                    logger.info(f"[PREWARM] Installed bapX for new user {user_id[:8]}...")
            except Exception as e:
                logger.debug(f"[PREWARM] Could not ensure bapX for {user_id[:8]}...: {e}")
